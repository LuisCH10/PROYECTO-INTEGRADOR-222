...
public class CRUDdetalles extends ConectarBD{
    ...
    public void InsertarRegistroDetalles(double subtotal,double descuento, double montofinal, double costodelivery,String direccion) {
        try {
            this.obtenerconexion();
            // preparedStatament permite especificar parametros en la consulta a traves del simbolo la interrogante (?)
            ps = conexion.prepareStatement("INSERT INTO detalles (subtotal,descuento,montofinal,costodelivery,direcciondelivery) "
                    + "VALUES("+subtotal+","+descuento+","+montofinal+","+costodelivery+",'"+direccion+"');");
            ...
            Mensajes.M1("ERROR! No se puede insertar el registro... AQUI" + ex);
        } finally {
            this.cerrarconexion();
        }
    }
